...
def recomendar_canciones_lista(cancion):
    lista_retorno = []
    recommended_songs = get_recommendations(cancion, N=10)
    if isinstance(recommended_songs, str):
        print(recommended_songs)
    else:
        for son in recommended_songs:
            listatemp = []
            artista = son['artists']
            if ";" in artista:
                artista = artista.split(";")[0]
                
            listatemp.append(artista)
            listatemp.append(son['track_name'])
            lista_retorno.append(listatemp)
    return lista_retorno

app = Flask(__name__)
CORS(app)  # Habilita CORS para tu aplicación Flask

@app.route('/modelo3', methods=['POST'])
def modelo1():
    try:
